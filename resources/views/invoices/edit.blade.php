    ...
    <script>
        function invoiceForm() {
            return {
                isSubmitting: false,
                form: {
                    invoice_date: "{{ old('invoice_date', $invoice->invoice_date) }}",
                    due_date: "{{ old('due_date', $invoice->due_date) }}",
                    discount_percent: {{ old('discount_percent', $invoice->discount_percent) }},
                    tax_percent: {{ old('tax_percent', $invoice->tax_percent) }},
                    from_name: "{{ old('from_name', $invoice->from_name) }}",
                    from_address: "{{ old('from_address', $invoice->from_address) }}",
                    to_name: "{{ old('to_name', $invoice->to_name) }}",
                    to_address: "{{ old('to_address', $invoice->to_address) }}",
                    items: [
                        @foreach($invoice->items as $item)
                        {
                            quantity: {{ old('items.'.$loop->index.'.quantity', $item->quantity) }},
                            description: "{{ old('items.'.$loop->index.'.description', $item->description) }}",
                            price: {{ old('items.'.$loop->index.'.price', $item->price) }},
                            amount: {{ $item->amount }}
                        },
                        @endforeach
                    ],
